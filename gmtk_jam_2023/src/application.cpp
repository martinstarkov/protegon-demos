...
public:
	...
	SolidButton start_wave_button{ Rectangle<int>{ { 0, map_size.y - 50 }, { 100, 50 } }, color::DarkGrey, color::Black, color::Black };
	bool paused = false;
	...
	void Update(float dt) final {
		...
		*/
		...
		Rectangle<float> full_end_bar{ {window_size.x / 2 - full_end_bar_size.x / 2, 0 }, full_end_bar_size };
		...
		health_bar_border.Draw(color::DarkBrown, 6);
		...
		queue_frame_border.Draw(color::DarkBrown, 6);
		...
		Rectangle<int> money_text_box{ {window_size.x - money_text_size.x - 5, 0 }, { money_text_size.x, money_text_size.y } };
...
public:
	...
	void Update(float dt) final {
		...
		t.Draw(Rectangle<int>{ play_text_pos - V2_int{ 250, 160 }, { play_text_size.x + 500, play_text_size.y } });

		Text t2{ Hash("2"), "'b' between waves to open purchase menu", color::Brown };
		t2.Draw(Rectangle<int>{ play_text_pos - V2_int{ 250, 160 - 70 }, { play_text_size.x + 500, play_text_size.y } });
		
		Text t3{ Hash("2"), "'Space' to send the units on their way", color::DarkGrey };
		t3.Draw(Rectangle<int>{ play_text_pos - V2_int{ 250, 160 - 70 - 70 }, { play_text_size.x + 500, play_text_size.y } });

		Text t4{ Hash("2"), "If units do not kill end goal, wave resets", color::Gold };
		t4.Draw(Rectangle<int>{ play_text_pos - V2_int{ 250, 160 - 70 - 70 - 70 }, { play_text_size.x + 500, play_text_size.y } });
...
public:
	...
	void Update(float dt) final {
		...
		menu_bg_border.Draw(color::DarkBrown, 20);
...
public:
	...
	void Update(float dt) final {
		...
		t3.Draw(Rectangle<int>{ play_text_pos - V2_int{ 250, 160 }, { play_text_size.x + 500, play_text_size.y } });
...
public:
	...
	void Update(float dt) final {
		...
		if (hover) {
			...
			button_hover.Draw(Rectangle<int>{ play_pos, play_size });
		} else {
			button.Draw(Rectangle<int>{ play_pos, play_size });
		}
